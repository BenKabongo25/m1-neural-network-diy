...
class MaxPool1D(module.Module):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        self._amax = 1. * (X_col == np.amax(X_col, axis=2, keepdims=True))
        Z = X_col.max(axis=2)
        return Z
    ...
    def backward_delta(self, X: np.ndarray, delta: np.ndarray) -> np.ndarray:
        ...
        tmp = (
            np.tile(delta, 2) * 
            self._amax.reshape(batch_size, self._out_length, -1)
        ).reshape(batch_size, self._out_length, self.k_size, chan) / batch_size
